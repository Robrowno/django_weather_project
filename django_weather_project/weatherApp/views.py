...
def home(request):
    ...
    forcast_weather_url = "https://api.openweathermap.org/data/2.5/onecall?lat={}&lon={}&units=metric&appid={}&exclude=current,minutely,hourly,alerts"
...
def fetch_weather_and_forecast(city, api_key, current_weather_url, forcast_weather_url):
    """Fetch weather and forecast data from openweathermap.org"""

    # Fetch current weather data
    current_weather = requests.get(current_weather_url.format(city, api_key)).json()
    # Fetch forecast weather data
    forecast_weather = requests.get(forcast_weather_url.format(current_weather['coord']['lat'], current_weather['coord']['lon'], api_key)).json()
    ...
    for data in forecast_weather['daily'][:5]:
        daily_forecast.append({
            ...
            'temperature': round(data['temp']['day'] - 273.15, 2),
